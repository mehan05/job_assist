...
export default function CreateJobPage({params}:{params:{id:string}}) {
  ...
  const getDataForEdit = async(id:string)=>{
    ...
    try {
        const response = await axios.get("/api/company-api/post-job/edit/"+id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.put(`http://localhost:3000/api/company-api/post-job/edit/${id}`,jobData);
  ...
  const HandleSkillAdd = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === "Enter") {
      ...
      {
        setskillsRequired((prev)=>[...jobData.skillsRequired,...prev,skill]); 
        setSkill("");
      }
      if(skill.trim() && skillsRequired.includes(skill.trim()))
      {
